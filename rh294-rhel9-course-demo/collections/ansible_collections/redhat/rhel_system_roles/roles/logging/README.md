...
`redhat.rhel_system_roles` collection, if `logging_manage_firewall`
and `logging_manage_selinux` is set to true, respectively.
(Please see also the variables in the `Other options` section.)

If the `logging` is a role from the `redhat.rhel_system_roles`
