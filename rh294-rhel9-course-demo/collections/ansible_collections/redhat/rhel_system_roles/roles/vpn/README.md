...
`redhat.rhel_system_roles` collection, if `vpn_manage_firewall`
and `vpn_manage_selinux` is set to true, respectively.
(Please see also the variables in the `Firewall and Selinux` section.)

If the `vpn` is a role from the `redhat.rhel_system_roles`
